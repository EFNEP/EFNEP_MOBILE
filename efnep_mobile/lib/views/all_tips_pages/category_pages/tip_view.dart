...
import 'package:efnep_mobile/views/widgets/tip_card_view.dart';
...
class TipView extends StatefulWidget {
  final String cat;
  final String subCat;
  const TipView({Key? key, required this.cat, required this.subCat})
      : super(key: key);
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
        if (snapshot.hasError) {
          return const Text(wentWrong, style: TextStyle( fontWeight: FontWeight.w500),);
        }
        ...
        if (snapshot.data!.docs.isEmpty) {
          return const Center(
            child: Text(noData, style:  TextStyle( fontWeight: FontWeight.w500),),
          );
        }
