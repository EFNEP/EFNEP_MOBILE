...
class PollCommentCard extends StatefulWidget {
  final snap;
  final String pollId;
  const PollCommentCard({Key? key, required this.snap, required this.pollId})
      : super(key: key);

  @override
  State<PollCommentCard> createState() => _PollCommentCardState();
}

class _PollCommentCardState extends State<PollCommentCard> {
  ...
  void postPollReply(String commentId) async {
    try {
      String res = await FireStoreMethods().postPollReply(
        widget.pollId,
  ...
  void loadReplies() async {
    try {
      List<Map<String, dynamic>> replyData = await FireStoreMethods()
          .getPollReplies(widget.pollId, widget.snap.data()['commentId']);
